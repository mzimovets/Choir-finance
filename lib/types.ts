...
export interface Member {
  ...
  /**
   * Чтец по умолчанию: подставляется в новый выход сам. Если звёздочки нет
   * ни у кого, чтец не подставляется и выбирается вручную.
   */
  isPreferredReader?: boolean
}
...
export interface Attendance {
  ...
  isReader?: boolean
  /**
   * Множитель ставки: 0.5 — пришёл на половину, 1 (или поле отсутствует) — целиком,
   * 2 и больше — двойная (и т.д.) ставка. basePrice уже посчитан с учётом множителя.
   */
  share?: number
  /**
   * Полная ставка до применения доли. Сохраняется вместе с share, чтобы в
   * табеле стояла формула с настоящей ставкой («500/3»), а не восстановленной
   * обратным делением.
   */
  fullPrice?: number
}
