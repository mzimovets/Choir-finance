...
export default function ExportPage() {
  ...
  // Настройки хора — влияют на состав табеля и ведомости
  useEffect(() => {
    fetch("/api/settings")
      .then((r) => (r.ok ? r.json() : null))
      .then((s) => { if (s) setHideZeroMembers(!!s.hideZeroMembers); })
      .catch(() => {});
  }, []);
  ...
  async function updateHideZero(next: boolean) {
    ...
    await fetch("/api/settings", {
      ...
      body: JSON.stringify({ hideZeroMembers: next }),
    });
  }
  ...
  function handleCellPointerUp(e: React.PointerEvent, ev: ChoirEvent, memberId: string) {
    ...
    router.push(
      `/day?date=${ev.date}&event=${encodeURIComponent(ev._id)}&member=${encodeURIComponent(memberId)}`,
    );
  }

  /** Пустые ячейки («—») не нажимаются: певчего в этом выходе не было */
  const cellTapProps = (ev: ChoirEvent, memberId: string, hasValue: boolean) =>
    hasValue
      ? {
          onPointerDown: handleCellPointerDown,
          onPointerUp: (e: React.PointerEvent) => handleCellPointerUp(e, ev, memberId),
  ...
  const activeMembers = members
    ...
    // Настройка «скрывать без выплат»: без единого рубля за месяц певчий
    // не попадает в табель и ведомость, но остаётся в «Итогах»
    .filter((mb) => !hideZeroMembers || monthTotalOf(mb._id) !== 0)
  ...
  return (
    <div className="max-w-screen-lg mx-auto">
            ...
            <div className={isFullscreen ? "flex-1 flex flex-col overflow-clip" : "warm-card overflow-clip"}>
              ...
              {(() => {
                ...
                return (
                  <>
                    ...
                    <div style={{ display: "flex", alignItems: "center", background: C_HEAD_BG, borderBottom: `1px solid ${C_BORDER}` }}>
              ...
              <div
                ...
                style={{
                  ...
                  overflow: "auto",
                  maxHeight: wrapMaxH,
                  // Только там, где таблица прокручивается сама: none запрещает и
                  // резиновый отскок у края, и передачу прокрутки наружу. На компьютере
                  // это бы намертво блокировало колесо над таблицей — страница
                  // не листалась бы вовсе, ведь по вертикали блок не прокручивается.
                  overscrollBehavior: tableScrollsInside ? "none" : undefined,
                      ...
                      return (
                        <tr key={mb._id}>
                          ...
                          {sortedEvents.map((ev, idx) => {
                            ...
                            return (
                              <td key={ev._id} {...cellTapProps(ev, mb._id, val !== null)} style={{ ...tdBase, position: "relative", overflow: "hidden", background: evenRow ? "#fdf8f4" : C_BG, fontWeight: val ? 600 : 400, color: val ? C_TEXT : "#d4c0ac", fontSize: 12, cursor: val !== null && !selectionMode ? "pointer" : undefined, borderRight: isGroupEnd && idx < sortedEvents.length - 1 ? `1px solid ${C_SEP}` : undefined }}>
      ...
      {/* ── Модалка группы ── */}
      {/* Настройки табеля и ведомости */}
      {settingsOpen && (
        <>
          <div className="fixed inset-0 z-50 bg-black/50" onClick={() => setSettingsOpen(false)} />
          <div className="fixed inset-0 z-50 flex items-center justify-center px-5" onClick={() => setSettingsOpen(false)}>
            <div className="bg-white rounded-2xl w-full max-w-sm shadow-2xl overflow-hidden" onClick={(e) => e.stopPropagation()}>
              <div className="px-5 pt-5 pb-3 flex items-center justify-between border-b border-warm-100">
                <h2 className="text-base font-slab font-bold text-warm-900">Настройки</h2>
                <button
                  onClick={() => setSettingsOpen(false)}
                  className="w-8 h-8 rounded-xl border border-warm-200 bg-white text-warm-500 flex items-center justify-center"
                >
              ...
              <button
                onClick={() => updateHideZero(!hideZeroMembers)}
                className="w-full px-5 py-4 flex items-start gap-3 text-left active:bg-warm-50"
              >
                <div
                  className="mt-0.5 shrink-0 rounded-full transition-colors"
                  style={{
                    width: 42, height: 24, padding: 3,
                    background: hideZeroMembers ? C_ACCENT : "#dcd0c2",
                    display: "flex", justifyContent: hideZeroMembers ? "flex-end" : "flex-start",
                  }}
                >
                        ...
                        return (
                          <tr key={mb._id}>
                            ...
                            {sortedEvents.map((ev, idx) => {
                              ...
                              for (const g of dateGroups) { cumIdx += g.count; if (idx === cumIdx - 1) { isGroupEnd = true; break; } if (idx < cumIdx) break; }
                              return (
                                <td key={ev._id} {...cellTapProps(ev, mb._id, val !== null)} style={{ ...tdBase, background: evenRow ? "#fdf8f4" : C_BG, fontWeight: val ? 600 : 400, color: val ? C_TEXT : "#d4c0ac", fontSize: 12, cursor: val !== null ? "pointer" : undefined, borderRight: isGroupEnd && idx < sortedEvents.length - 1 ? `1px solid ${C_SEP}` : undefined }}>
                                  {val !== null ? val.toLocaleString("ru-RU") : "—"}
                                </td>
                              );
                            })}
