...
import { hasPin, markPinVerified, clearPin } from '@/lib/pin'
...
export default function LoginPage() {
  ...
  // Шаг 2: выбор хора
  async function handleSelectChoir(choirType: 'festive' | 'weekday') {
    setSelectingChoir(choirType)
    setLoading(true)
    try {
      const res = await fetch('/api/auth/select-choir', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ verifyToken, choirType }),
      })
      if (!res.ok) {
        setError('Ошибка при выборе хора')
        setStep('credentials')
        return
      }
      router.replace('/day')
  ...
  if (step === 'pin-lock') {
    return (
      <PinLock
        onUnlock={() => { markPinVerified(); setStep('choir') }}
        onCancel={() => setStep('credentials')}
        onForgotPin={() => { clearPin(); setStep('pin-setup') }}
      />
    )
  }
  ...
  if (step === 'pin-setup') {
    return (
      <PinSetup
        onDone={() => { markPinVerified(); setStep('choir') }}
        onCancel={() => setStep('credentials')}
      />
    )
  }
        ...
        <div className="warm-card p-6">
          {step === 'credentials' ? (
            ...
            <form onSubmit={handleVerify} className="flex flex-col gap-4">
              <p className="text-center text-xl font-slab font-bold text-warm-900">Авторизация</p>
              <div>
              ...
              <button
                type="submit"
                disabled={loading}
                className="btn-gradient w-1/2 mx-auto py-3 text-base mt-1 flex items-center justify-center gap-2"
              >
            ...
            <div className="flex flex-col gap-4">
              ...
              <div className="flex flex-col gap-3">
                <button
                  onClick={() => handleSelectChoir('festive')}
                  disabled={loading}
                  className="w-full py-4 rounded-2xl border-2 border-warm-200 text-warm-900 font-slab font-bold text-base text-left px-5 flex items-center gap-3 active:bg-warm-50 transition-colors disabled:opacity-50"
                >
                  <span className="text-2xl">🕊</span>
                  Праздничный хор
                  {selectingChoir === 'festive' && <span className="ml-auto"><LoadingSpinner size="sm" color="#9b7653" /></span>}
                </button>
                <button
                  onClick={() => handleSelectChoir('weekday')}
                  disabled={loading}
                  className="w-full py-4 rounded-2xl border-2 border-warm-200 text-warm-900 font-slab font-bold text-base text-left px-5 flex items-center gap-3 active:bg-warm-50 transition-colors disabled:opacity-50"
                >
                  <span className="text-2xl">📅</span>
                  Будний хор
                  {selectingChoir === 'weekday' && <span className="ml-auto"><LoadingSpinner size="sm" color="#9b7653" /></span>}
                </button>
              </div>
              ...
              <button
                ...
                className="w-full flex items-center justify-center gap-2 py-2.5 rounded-xl border border-warm-200 text-sm font-semibold text-warm-600 bg-warm-50 active:bg-warm-100 transition-colors mt-1"
              >
